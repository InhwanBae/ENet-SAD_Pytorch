...
class BDD100K(Dataset):
    ...
    def createIndex(self):
        listfile = os.path.join(self.data_dir_path, "list", "{}_gt_bdd.txt".format(self.image_set))
        ...
        with open(listfile) as f:
            for line in f:
                line = line.strip().replace("/train_img/", "/images/100k/", 1)
                l = line.split(" ")
    ...
    def __getitem__(self, idx):
        ...
        if self.image_set != 'test':
            segLabel = cv2.imread(self.segLabel_list[idx])[:, :, 0] / 255  # label image consists 0, 255
            # linewidth to 8
            # kernelNDArray = np.ones((8, 8), np.uint8)
            # segLabel = cv2.dilate(segLabel, kernelNDArray, iterations=1)
